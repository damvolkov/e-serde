...
fn to_node(value: toml::Value) -> Node {
    match value {
        ...
        toml::Value::Table(table) => {
            Node::Map(table.into_iter().map(|(k, v)| (k, to_node(v))).collect())
        }
    }
}
...
fn decode(input: &str) -> Result<Node, String> {
    ...
    Ok(Node::Map(
        table.into_iter().map(|(k, v)| (k, to_node(v))).collect(),
    ))
}
...
fn from_node(node: Node) -> Result<toml::Value, String> {
    Ok(match node {
        ...
        Node::BigInt(digits) => match digits.parse::<i64>() {
            ...
            Err(_) => {
                return Err(format!(
                    "TOML cannot hold the integer {digits} (beyond its 64-bit range)"
                ));
            }
        },
        ...
        Node::Seq(items) => {
            toml::Value::Array(items.into_iter().map(from_node).collect::<Result<_, _>>()?)
        }
        Node::Map(entries) => {
