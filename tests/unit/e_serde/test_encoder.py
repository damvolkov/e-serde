...
from e_serde.logic.encoder import encode
...
class _Vec(msgspec.Struct, frozen=True):
    x: int
    y: int
...
async def test_encode_dict_none_key() -> None:
    assert encode({None: 1}) == {"null": 1}
...
async def test_encode_uuid() -> None:
    uid = UUID("12345678-1234-5678-1234-123456781234")
    assert encode(uid) == "12345678-1234-5678-1234-123456781234"
...
async def test_encode_path() -> None:
    assert encode(PurePosixPath("/srv/app/data")) == "/srv/app/data"
...
async def test_encode_intenum() -> None:
    assert encode(_Int.ONE) == 1
...
async def test_encode_msgspec_struct_frozen() -> None:
    assert encode(_Vec(1, 2)) == {"x": 1, "y": 2}
...
async def test_encode_object_with_model_dump() -> None:
    class _FakeModel:
        def model_dump(self, *, mode: str = "python") -> dict[str, object]:
            return {"mode": mode, "a": 1}

    assert encode(_FakeModel()) == {"mode": "json", "a": 1}


async def test_encode_jsonable_key_from_sequence() -> None:
    assert encode({(1, 2): "v"}) == {"[1, 2]": "v"}


async def test_encode_jsonable_key_from_textual() -> None:
    uid = UUID("12345678-1234-5678-1234-123456781234")
    assert encode({uid: "v"}) == {"12345678-1234-5678-1234-123456781234": "v"}
